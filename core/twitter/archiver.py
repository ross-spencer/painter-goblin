...
def format_date(utc, isoformat=False):
    ...
    # welcome.
    unew = datetime.combine(
        u.date(), time(u.time().hour, u.time().minute, u.time().second, tzinfo=UTC)
    )
...
def statuses(
    ...
    while True:
        try:
            ...
            if e.e.code == 401:
                err(
                    "Fail: %i Unauthorized (tweets of that user are protected)"
                    % e.e.code
                )
                break
            elif e.e.code == 429:
                ...
                delay = int(rls.rate_limit_reset - _time.time()) + 5  # avoid race
                err(
            ...
            new += len(tweets)
            err(
                "Browsing %s statuses, new tweets: %i"
...
def main(args=sys.argv[1:]):
    ...
    if options["oauth"]:
        ...
        if not os.path.exists(oauth_filename):
            oauth_dance(
                "Twitter-Archiver", CONSUMER_KEY, CONSUMER_SECRET, oauth_filename
            )
    ...
    if options["timeline"] or options["mentions"]:
        ...
        try:
            ...
            err("Error when loading saved tweets: %s - continuing without" % str(e))
    ...
    if options["dms"]:
        ...
        try:
            ...
            err("Error when loading saved DMs: %s - continuing without" % str(e))

        try:
            statuses(
                twitter, "", dms, received_dms=True, isoformat=options["isoformat"]
            )
            statuses(
                twitter, "", dms, received_dms=False, isoformat=options["isoformat"]
            )
        except KeyboardInterrupt:
            err()
            err("Interrupted")
    ...
    print("Total: %i tweets (%i new) for %i users" % (total, total_new, len(users)))
