...
PREFIXES = dict(
    cats=dict(new_tweet="=^_^= ", error="=O_o= ", inform="=o_o= "),
    none=dict(new_tweet=""),
)
...
class TwitterBot(object):
    def __init__(self, configFilename):
        ...
        self.sched = Scheduler(
            (SchedTask(self.process_events, 1), SchedTask(self.check_statuses, 120))
        )
        ...
        try:
            ...
            if args[0] == "follow" and args[1:]:
                ...
                conn.privmsg(
                    ...
                    "(unfollow <twitter_name>) to make me stop." % get_prefix(),
                )
        except Exception:
            traceback.print_exc(file=sys.stderr)
    ...
    def follow(self, conn, evt, name):
        ...
        if name in friends:
            conn.privmsg(
                userNick, "%sI'm already following %s." % (get_prefix("error"), name)
            )
        else:
            try:
    ...
    def unfollow(self, conn, evt, name):
        ...
        if name not in friends:
            conn.privmsg(
                userNick, "%sI'm not following %s." % (get_prefix("error"), name)
            )
        else:
